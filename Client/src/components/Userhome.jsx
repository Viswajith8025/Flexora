                  ...
                  <div className="space-y-8">
                     {[
                        ...
                        <div key={i} className="flex items-start gap-5 relative">
                           <div className={`w-9 h-9 rounded-xl flex items-center justify-center border shrink-0 z-10 ${step.status === 'done'
...
const FlexoraDashboard = () => {
   ...
   useEffect(() => {
      if (currentUser) {
         ...
         // 📍 High-Fidelity Localized Recommendations
         async function fetchRecommendations() {
            try {
               ...
               console.error("Recommendation Sync Error:", err.message);
               // Fallback: If recommendation endpoint fails, show recent jobs
               ...
               <div className="lg:col-span-2 space-y-12">
                              ...
                              {myApplications.map((app, i) => (
                                 ...
                                 >
                                    ...
                                    <div className="flex items-center gap-4 shrink-0">
                                       <div className={`px-4 py-1.5 rounded-lg flex-meta border shadow-sm ${app.status === 'accepted'
                  ...
                  {currentUser?.role === 'job_seeker' && (
                     <section>
                        <div className="flex-section-header">
                           <h3 className="flex-label text-white flex items-center gap-2">
                              <Sparkles size={14} className="text-blue-500" /> Recommended in {currentUser?.district || 'your Region'}
                           </h3>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                 ...
                                 <div key={applicant.user?._id || i} className="flex-card p-6 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-8 group hover:border-slate-700 transition-colors mb-4 last:mb-0">
                                    ...
                                    <div className="flex items-center gap-3 w-full md:w-auto">
                                       <button
                                          ...
                                          className={`flex-1 md:flex-none !px-8 !py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${applicant.status === 'accepted'
                                       ...
                                       <button
                                          ...
                                          className={`flex-1 md:flex-none !px-8 !py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${applicant.status === 'rejected'
            ...
            {selectedApplicationDetails && (
               <div key="application-detail-overlay" className="fixed inset-0 z-[200] flex items-center justify-center p-6">
                  ...
                  <motion.div key="detail-modal" initial={{ opacity: 0, scale: 0.97, y: 8 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.97, y: 8 }} transition={{ duration: 0.22, ease: [0.16, 1, 0.3, 1] }} className="relative w-full max-w-xl bg-slate-900 border border-slate-800 rounded-[40px] overflow-hidden shadow-2xl p-10 sm:p-14">
                     <div className="flex justify-between items-start mb-10">
                        <div className={`px-4 py-1.5 rounded-lg flex-meta border ${selectedApplicationDetails.status === 'accepted'
                           ? "bg-green-600/10 text-green-500 border-green-500/20"
                     ...
                     <div className="flex-card flex-card-padding bg-slate-950/50 mb-10">
                        <div className="flex justify-between items-center mb-6 pb-6 border-b border-slate-900">
                           ...
                           <span className={`font-black uppercase tracking-widest text-xs ${selectedApplicationDetails.status === 'accepted' ? "text-green-500" : selectedApplicationDetails.status === 'rejected' ? "text-red-500" : "text-blue-500"
                              }`}>
